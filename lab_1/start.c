...
void runJacobyMethod (float *rho, float* phi) {
    ...
    while (iterNumber <= N_t) {
        ...
        printf("[%d]globalDelta=%f\tstepDelta=%f\n", iterNumber, globalDelta, stepDelta);
    ...
    if (isSuccess) printf("Jacoby method finished\n");
    else printf("Jacoby method failed\n");
...
int main() {
    ...
    initRho(rho);
    // fillFile(rho, "rho.dat");

    float *phi = (float*)malloc(N_x * N_y * sizeof(float));
    runJacobyMethod(rho, phi);
    // fillFile(phi, "phi.dat");
